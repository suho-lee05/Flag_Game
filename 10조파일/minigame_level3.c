...
void start_avoid_F_level3()
{
    ...
    {
        cls;
        gotoxy_1_level3(15, 10);
        printf("Game OVER!\n");
        gotoxy_1_level3(12, 11);
        printf("당신의 점수는 %d점입니다", score);
        gotoxy_1_level3(10, 12);
        system("pause");
        CursorView(1);
...
int playGame_level3(int* life)
{
    ...
    while (1)
    {
        gotoxy_1_level3(25, 4);
        printf("Score: %d", score); // 점수 출력
        gotoxy_1_level3(25, 5);
        printf("Life: %d", *life);  // 목숨 출력

        // F 만드는 부분
        if (count % LEVEL_3 == 1)      // 루프가 돌면서 변수 count가 증가
...
void guessNumberGame_level3()
{
    ...
    for (int i = 0; i < 1; i++)
    {
        ...
        if (attempts == 4 && userGuess != targetNumber)
        {
            printf("오답입니다! 정답은 %d 입니다!\n", targetNumber);
        }
    }

    printf("종료되었습니다．성적은：%d\n", score);
    Sl; Sl;
    switch (score)
    {
    case 0:
...
void mysteryQuiz_level3()
{
    char c, ans[9] = { 0 };
    int a, i, count = 0;
    int t1 = time(0), t2 = time(0) + 5;            //t2에 더하는 수가 제한시간
...
int time_attack_game_level3()
{
    ...
    printf("측정 시작!\n");
    sound();
    start_time = get_current_time_in_ms_level3();
    printf("측정 중... (엔터 키를 눌러서 종료하세요)\n");
    print_time_in_seconds_level3(0, time_over);

    while (!_kbhit())    //측정중...
    {
        Sleep(100);
        elapsed_time = get_current_time_in_ms_level3() - start_time;
        time_over = print_time_in_seconds_level3(elapsed_time);
        if (time_over == 1)
        {
...
void matching_arrow_level3()
{
    ...
    int rand_arrow[10] = { 0 };
    int match = 0;
    int t1 = time(0), t2 = time(0) + 5;            //t2에 더하는 숫자가 제한시간
    int x = 40, y = 20;
