...
void start_avoid_F_level2()
{
    ...
    {
        draw_pan();
        gotoxy_1_level2(5, 11);
        printf("아무키나 누르면 시작합니다");
        system("pause > nul");       // 화면 일시정지 시켜주는 것
        gotoxy_1_level2(5, 11);
        printf("                          ");

        score += playGame_level2(&life); // 게임 실행 및 점수 합산
    }
    if (score < DEAD_SCORE)
    {
        cls;
        gotoxy_1_level2(15, 10);
        printf("Game OVER!\n");
        gotoxy_1_level2(12, 11);
        printf("당신의 점수는 %d점입니다", score);
        gotoxy_1_level2(10, 12);
        system("pause");
        CursorView(1);
...
int playGame_level2(int* life)
{
    ...
    while (1)
    {
        gotoxy_1_level2(25, 4);
        printf("Score: %d", score); // 점수 출력
        gotoxy_1_level2(25, 5);
        printf("Life: %d", *life);  // 목숨 출력

        // F 만드는 부분
        if (count % LEVEL_2 == 1)      // 루프가 돌면서 변수 count가 증가    //f빈도
...
void guessNumberGame_level2()
{
    ...
    for (int i = 0; i < 1; i++)
    {
        ...
        if (attempts == 5 && userGuess != targetNumber)
        {
...
void mysteryQuiz_level2()
{
    char c, ans[9] = { 0 };
    int a, i, count = 0;
    int t1 = time(0), t2 = time(0) + 10;            //t2에 더하는 수가 제한시간
...
int time_attack_game_level2()
{
    ...
    printf("측정 시작!\n");
    sound();
    start_time = get_current_time_in_ms_level2();
    printf("측정 중... (엔터 키를 눌러서 종료하세요)\n");
    print_time_in_seconds_level2(0, time_over);

    while (!_kbhit())    //측정중...
    {
        Sleep(100);
        elapsed_time = get_current_time_in_ms_level2() - start_time;
        time_over = print_time_in_seconds_level2(elapsed_time);
        if (time_over == 1)
        {
...
void matching_arrow_level2()
{
    ...
    int rand_arrow[10] = { 0 };
    int match = 0;
    int t1 = time(0), t2 = time(0) + 7;            //t2에 더하는 숫자가 제한시간
    int x = 40, y = 20;
